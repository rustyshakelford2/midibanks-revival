...
public class SongInstance {
	...
	protected SongInstance(MidiBanks plugin, org.bukkit.block.Sign midiSign,
		...
		/* 45 */BlockFace direction = ((org.bukkit.material.Sign) midiSign
				.getData()).getFacing();
		/* 46 */if (direction == BlockFace.NORTH)
			this.sx = 1;
		/* 47 */if (direction == BlockFace.SOUTH)
			this.sx = -1;
		/* 48 */if (direction == BlockFace.EAST)
			this.sz = 1;
		/* 49 */if (direction == BlockFace.WEST)
			this.sz = -1;
		/* 50 */this.firstBlock = midiSign.getBlock().getRelative(this.sx * 2,
				0, this.sz * 2);

		/* 52 */this.latestNote = new int[16];
		/* 53 */for (int i = 0; i < this.latestNote.length; i++)
			this.latestNote[i] = -1;

		/* 55 */if (ochans != null) {
			/* 56 */this.chans = "";
			/* 57 */for (int i = 0; i < ochans.length(); i++) {
				/* 58 */if (ochans.charAt(i) == 'l')
					this.chans += "0123456789abcdef";
				else
					/* 59 */this.chans += ochans.charAt(i);
				/* 60 */if ((ochans.charAt(i) == 's')
						|| (ochans.charAt(i) == 'm')) {
					/* 61 */plugin.pinHandler.outputPin(
			...
			/* 64 */this.chans = this.chans.substring(0,
					32 < this.chans.length() ? 32 : this.chans.length());
		}
	}
	...
	protected void nextTick() {
		...
		for (; (this.event < this.music.size() - 1)
					...
					if ((si.equals(Integer.toHexString(channel)))
						...
						/* 89 */relBlocks.add(this.midiSign.getBlock()
								.getRelative(this.sx * (i + 2), 0,
										this.sz * (i + 2)));
					/* 90 */if (si.equals("n"))
						/* 91 */this.plugin.pinHandler.outputPin(
			...
			/* 101 */int midiNote = this.music.get(this.event).getMessage()
					.getMessage()[1];
			...
			/* 103 */if (this.remRepeated) {
				...
				/* 109 */if (this.window < 0) {
					...
					/* 112 */Integer iaux11;
					/* 112 */if ((midiNote >= keyNote)
							&& (midiNote < keyNote + 24)) {
						iaux11 = Integer.valueOf(midiNote - keyNote);
					} else {
						/* 113 */Integer iaux111;
						/* 113 */if ((this.repOctave) && (midiNote < keyNote)) {
							iaux111 = Integer.valueOf((midiNote - 6) % 12);
						} else {
							/* 114 */Integer iaux1111;
							/* 114 */if ((this.repOctave)
									&& (midiNote >= keyNote + 24))
								iaux1111 = Integer
										.valueOf(12 + (midiNote + 6) % 12);
							else
								/* 115 */iaux1111 = Integer.valueOf(-1);
						}
					}
				}
				...
				/* 119 */for ( Block relBlock : relBlocks) {
					...
					try {
						...
						/* 123 */if (this.instrument.intValue() > -1) {
	...
	private void over() {
		...
		/* 146 */if (theend) {
			/* 147 */for (int i = 0; i < this.chans.length(); i++)
				/* 148 */if ((this.chans.charAt(i) == 'z')
						|| (this.chans.charAt(i) == 'm'))
					/* 149 */this.plugin.pinHandler.outputPin(
