   ...
   protected void nextTick() {
     ...
     for (; (this.event < this.music.size() - 1) && (this.music.get(this.event).getTick() <= this.tick); this.event += 1) {
       ...
       else
       {
         ...
         Integer iaux1 = 0;
         if (this.window < 0) {
           iaux1 = Integer.valueOf((midiNote + (this.shift ? 6 : -6)) % 24);
         }
         else
         {
           Integer iaux2;
           if ((midiNote >= keyNote) && (midiNote < keyNote + 24)) { iaux2 = Integer.valueOf(midiNote - keyNote);
           }
           else
           {
             Integer iaux3;
             if ((this.repOctave) && (midiNote < keyNote)) { iaux3 = Integer.valueOf((midiNote - 6) % 12);
             }
             else
             {
               Integer iaux4;
               if ((this.repOctave) && (midiNote >= keyNote + 24)) iaux4 = Integer.valueOf(12 + (midiNote + 6) % 12); else
         ...
         for (Block relBlock : relBlocks) {
           ...
           try {
             ...
             if (this.instrument.intValue() > -1) { nb.play(this.instrument.byteValue(), iaux1.byteValue());
             } else {
